...
import withAttr from 'duroom/utils/withAttr';
...
export default class SuspendUserModal extends Modal {
  ...
  title() {
    return app.translator.trans('duroom-suspend.forum.suspend_user.title', { user: this.attrs.user });
  }
      ...
      <div className="Modal-body">
        <div className="Form">
          <div className="Form-group">
            <label>{app.translator.trans('duroom-suspend.forum.suspend_user.status_heading')}</label>
            <div>{this.formItems().toArray()}</div>
          </div>

          <div className="Form-group">
            <Button className="Button Button--primary" loading={this.loading} type="submit">
              {app.translator.trans('duroom-suspend.forum.suspend_user.submit_button')}
            </Button>
          </div>
        </div>
      </div>
  ...
  radioItems() {
    ...
    items.add(
      ...
      <label className="checkbox">
        ...
        {app.translator.trans('duroom-suspend.forum.suspend_user.not_suspended_label')}
      </label>,
      100
    );
    ...
    items.add(
      ...
      <label className="checkbox">
        ...
        {app.translator.trans('duroom-suspend.forum.suspend_user.indefinitely_label')}
      </label>,
      90
    );
    ...
    items.add(
      ...
      <label className="checkbox SuspendUserModal-days">
        ...
        />
        {app.translator.trans('duroom-suspend.forum.suspend_user.limited_time_label')}
        {this.status() === 'limited' && (
          <div className="SuspendUserModal-days-input">
            ...
            {app.translator.trans('duroom-suspend.forum.suspend_user.limited_time_days_text')}
          </div>
        )}
      </label>,
      80
  ...
  formItems() {
    ...
    items.add(
      ...
      <div className="Form-group">
        <label>
          {app.translator.trans('duroom-suspend.forum.suspend_user.reason')}
          <textarea
            className="FormControl"
            bidi={this.reason}
            placeholder={app.translator.trans('duroom-suspend.forum.suspend_user.placeholder_optional')}
            rows="2"
          />
        </label>
      </div>,
    ...
    items.add(
      ...
      <div className="Form-group">
        <label>
          {app.translator.trans('duroom-suspend.forum.suspend_user.display_message')}
          <textarea
            className="FormControl"
            bidi={this.message}
            placeholder={app.translator.trans('duroom-suspend.forum.suspend_user.placeholder_optional')}
